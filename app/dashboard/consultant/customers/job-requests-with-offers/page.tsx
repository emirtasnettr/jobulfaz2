/**
 * Teklifteki İş İlanları Sayfası
 * 
 * Consultant'ların reddettiği ve yeni teklif verdiği iş ilanlarını görüntülediği sayfa
 */
...
export default function JobRequestsWithOffersPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Yeni teklif verilen iş ilanlarını al (status = 'REJECTED' ve rejection_reason = 'NEW_OFFER')
        const { data: jobs, error: jobsError } = await supabase
          .from('job_postings')
          .select('*')
          .eq('status', 'REJECTED')
          .eq('rejection_reason', 'NEW_OFFER')
  ...
  // Yeni teklif tutarını hesaplama (kişi başı)
  const getNewOfferPerPerson = (job: JobPosting) => {
    if (job.new_offer_monthly_budget_per_person) {
      return job.new_offer_monthly_budget_per_person;
    }
    if (job.new_offer_daily_budget_per_person) {
      return job.new_offer_daily_budget_per_person;
    }
    return null;
  };

  // Müşterinin eski teklifini hesaplama (KDV Hariç)
  const calculateOldOfferTotal = (job: JobPosting) => {
    if (job.job_type === 'FULL_TIME') {
      // Tam Zamanlı: Aylık maliyet gösterilir
      ...
      // Dönemsel: Toplam maliyet (ay sayısı * aylık)
      ...
      // Part-time: Toplam gün * günlük * kişi
  ...
  // Müşterinin eski teklifini kişi başı hesaplama (aylık/günlük)
  const getOldOfferPerPerson = (job: JobPosting) => {
    if (job.job_type === 'FULL_TIME' || job.job_type === 'SEASONAL') {
      return job.monthly_budget_per_person;
    } else if (job.job_type === 'PART_TIME') {
      return job.daily_budget_per_person;
    }
    return null;
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#F4F9FE' }}>
                ...
                <div className="absolute left-0 mt-2 w-56 bg-white/95 backdrop-blur-md rounded-lg border border-gray-200 shadow-lg overflow-hidden z-50">
                  <div className="py-1.5">
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">İş İlanı Talepleri</p>
                        <p className="text-xs text-gray-400">Onay bekleyen iş ilanları</p>
                      </div>
                    </Link>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">Aktif İş İlanları</p>
                        <p className="text-xs text-gray-400">Onaylanmış iş ilanları</p>
                      </div>
                    </Link>

                    <Link
                      href="/dashboard/consultant/customers/job-requests-with-offers"
                      onClick={() => setCustomerMenuOpen(false)}
                      className="w-full px-3 py-2.5 text-left flex items-center gap-2.5 hover:bg-gray-50 transition-colors"
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">Teklifteki İş İlanları</p>
                        <p className="text-xs text-gray-400">Yeni teklif verilen ilanlar</p>
                      </div>
                    </Link>

                    <Link
                      href="/dashboard/consultant/customers/active-contracts"
                      onClick={() => setCustomerMenuOpen(false)}
                      className="w-full px-3 py-2.5 text-left flex items-center gap-2.5 hover:bg-gray-50 transition-colors"
                    >
                      <div className="w-7 h-7 rounded-md bg-green-50 flex items-center justify-center">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">Aktif Sözleşmeler</p>
                        <p className="text-xs text-gray-400">Aday atanmış iş ilanları</p>
                      </div>
                    </Link>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">Geçmiş Sözleşmeler</p>
                        <p className="text-xs text-gray-400">Tamamlanmış iş ilanları</p>
                      </div>
                    </Link>
                  </div>
                </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Teklifteki İş İlanları</h1>
          <p className="text-sm text-gray-600">
            Reddettiğiniz ve yeni teklif verdiğiniz iş ilanları
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">Teklif Verilen İş İlanları ({jobPostings.length})</h3>
          </div>

          {jobPostings.length === 0 ? (
            <div className="p-12 text-center">
              <div className="text-gray-400 text-sm">
                Henüz teklif verilmiş iş ilanı bulunmuyor
              </div>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                        ...
                        <div>
                          <span className="text-gray-500">Teklif Tarihi:</span>
                          <p className="font-medium text-gray-900">{formatDate(job.rejected_at)}</p>
                        </div>
                        <div>
                      ...
                      {/* Eski Teklif Bilgileri */}
                      {(job.monthly_budget_per_person || job.daily_budget_per_person) && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4">
                          <h5 className="text-sm font-semibold text-gray-900 mb-3">Eski Teklif Özeti</h5>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="text-gray-600">
                                {job.job_type === 'PART_TIME' ? 'Günlük' : 'Aylık'} Kişi Başı Teklif (KDV Hariç):
                              </span>
                              <p className="font-medium text-gray-900 mt-1">
                                {getOldOfferPerPerson(job) ? formatCurrency(getOldOfferPerPerson(job)) : '-'}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">Toplam (KDV Hariç):</span>
                              <p className="font-medium text-gray-900 mt-1">
                                {calculateOldOfferTotal(job) ? formatCurrency(calculateOldOfferTotal(job)) : '-'}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">%20 KDV:</span>
                              <p className="font-medium text-gray-900 mt-1">
                                {calculateOldOfferTotal(job) ? formatCurrency((calculateOldOfferTotal(job) || 0) * 0.20) : '-'}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">Toplam (KDV Dahil):</span>
                              <p className="font-semibold text-gray-700 mt-1 text-base">
                                {calculateOldOfferTotal(job) ? formatCurrency((calculateOldOfferTotal(job) || 0) * 1.20) : '-'}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Yeni Teklif Bilgileri */}
                      {job.new_offer_total_without_vat && (
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
                          <h5 className="text-sm font-semibold text-gray-900 mb-3">Yeni Teklif Özeti</h5>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="text-gray-600">Kişi Başı Teklif (KDV Hariç):</span>
                              <p className="font-medium text-gray-900 mt-1">
                                {getNewOfferPerPerson(job) ? formatCurrency(getNewOfferPerPerson(job)) : '-'}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">Toplam (KDV Hariç):</span>
                              <p className="font-medium text-gray-900 mt-1">
                                {formatCurrency(job.new_offer_total_without_vat)}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">%20 KDV:</span>
                              <p className="font-medium text-gray-900 mt-1">
                                {formatCurrency((job.new_offer_total_without_vat || 0) * 0.20)}
                              </p>
                            </div>
                            <div>
                              <span className="text-gray-600">Toplam (KDV Dahil):</span>
                              <p className="font-semibold text-orange-700 mt-1 text-base">
                                {formatCurrency(job.new_offer_total_with_vat)}
                              </p>
                            </div>
                          </div>
                        </div>
