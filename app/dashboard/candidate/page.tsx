                  ...
                  {Array.from({ length: getDaysInMonth(currentYear, currentMonth) }, (_, i) => {
                    ...
                    return (
                      <div
                        ...
                        } ${jobsForDate.length > 0 ? 'hover:bg-blue-50' : ''}`}
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {jobsForDate.length > 0 && (
                            <span className="text-[10px] font-semibold text-blue-600 bg-blue-100 px-1 py-0.5 rounded">
                              {jobsForDate.length}
                            </span>
                          )}
                        </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="grid grid-cols-8 border-b border-gray-200 max-h-[700px] overflow-y-auto">
                  ...
                  {getWeekDays(currentWeek).map((day, idx) => {
                    ...
                    return (
                      <div key={idx} className={`border-r border-gray-200 last:border-r-0 relative ${isToday ? 'bg-blue-50/50' : 'bg-white'}`}>
                        {/* Gün başlığı */}
                        <div className={`h-14 border-b border-gray-200 flex flex-col items-center justify-center px-2 sticky top-0 z-10 ${isToday ? 'bg-blue-100 border-blue-300' : 'bg-gray-50'}`}>
          ...
          {selectedDate && jobDetailModalOpen && selectedJobAssignment && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
              <div className="bg-white rounded-xl shadow-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-5 py-3 flex items-center justify-between rounded-t-xl">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {selectedJobAssignment.job_postings?.city && selectedJobAssignment.job_postings?.district && (
                          <div className="bg-blue-50 rounded-lg px-3 py-2 border border-blue-100">
                            <p className="text-[10px] font-semibold text-blue-600 uppercase mb-0.5">Konum</p>
                                  ...
                                  {daysWithHours.map((day) => {
                                    ...
                                    return (
                                      <div key={day} className="bg-white rounded px-2.5 py-1.5 border border-indigo-200 flex items-center justify-between text-xs">
                                        ...
                                        <span className="font-semibold text-indigo-700">{diffHours}s{diffMins > 0 ? ` ${diffMins}d` : ''}</span>
                                      </div>
                                    );
                                  })}
                                </div>
